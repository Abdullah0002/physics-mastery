...
import type { KinematicsQuestion, Difficulty } from "./types";
...
export function KinematicsQuestionCard({
  question,
  index,
  bookmarked,
  onToggleBookmark,
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white shadow-sm transition-colors dark:border-slate-700 dark:bg-slate-900">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 p-4 dark:border-slate-800 sm:p-5">
        ...
        >
          <Bookmark
            className={`h-4 w-4 ${bookmarked ? "fill-amber-400 text-amber-500" : ""}`}
          />
          ...
          {question.options.map((opt, i) => {
            ...
            return (
              ...
              >
                ...
                {isSvg(opt) ? (
                  <SvgFigure svg={opt} className="max-w-[160px]" />
                ) : (
